...
int main()
{
	...
	std::string input_file = input_str;
	...
	u16 bytesPerSample = wavHeader.bitsPerSample / 8;
	u64 numSamples = wavHeader.Subchunk2Size / bytesPerSample;

	std::unique_ptr<i16[]> data = std::unique_ptr<i16[]>(new i16[2]);

	std::random_device device;
	std::uniform_real_distribution<> dist(0.0, std::nextafter(1.0, std::numeric_limits<f32>::max()));
	...
	print_data(wavHeader, numSamples);

	print_summary_text_file(wavHeader, start_time, stop_time, input_file, numSamples, max_channel_1_pre_noise, max_channel_2_pre_noise, max_channel_1_post_noise, max_channel_2_post_noise);
